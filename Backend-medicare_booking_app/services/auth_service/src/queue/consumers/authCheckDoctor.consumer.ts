...
export const initAuthCheckDoctorConsumer = async () => {
  const channel = getChannel();

  await channel.assertQueue("auth.checkDoctor", { durable: false });

  channel.consume("auth.checkDoctor", async (msg) => {
    if (!msg) return;
    ...
    try {
      ...
      channel.sendToQueue(
        ...
        Buffer.from(JSON.stringify({ isDoctor })),
        {
      ...
      console.error("Error processing auth.checkAdmin:", err);
